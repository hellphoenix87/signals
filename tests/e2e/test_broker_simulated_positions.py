...
- Simulated (backtest) positions must get distinct, non-None tickets so
...
- The simulated position dict's volume must be stored under a key that
  ...
  not "lot"), so dict-shaped simulated positions actually reach real
...
class TestBacktestModeSimulatedPositions:
    ...
    def test_simulated_position_reaches_real_loss_exit_logic_via_exit_trade(
        self, mock_mt5, make_tick
    ):
        """Full-stack proof: a real ExitTrade (composing the real
        LossExitManager/ProfitExitManager) must actually evaluate a
        simulated position's exit conditions -- not bail out silently
        ...
        broker.place_buy("EURUSD", 0.05, sl=1.0950, tp=1.1100, price=1.1000)
        ...
        assert len(actions) == 1, (
            ...
            "deeply-losing simulated position; got none -- volume/ticket likely "
            "still unreadable"
        )
        action = actions[0]
