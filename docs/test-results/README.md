...
**Also tried and rejected**: [ntick-confirmation-backtest.md](ntick-confirmation-backtest.md) — n-tick confirmation looked like a clear win in window 1 (every N beat baseline, improving with N) but flatly reversed in window 2 (underperformed baseline, below breakeven) — doesn't survive an out-of-sample check. `Config.N_TICK_CONFIRMATION` stays `1`. Also surfaced a real, still-open bug: confirmed signals bypass the session filter entirely (`SignalOrchestrator` calls `get_confirmed_signal()` directly, which `SessionFilteredSignalStrategy` never gates) — must be fixed before this is ever revisited.
