...
- **Second bug found while running the sweep (not part of the original plan, confirmed and fixed)**: `SessionFilteredSignalStrategy` only gated `generate_signal`, not `get_confirmed_signal` -- which `SignalOrchestrator._on_tick` calls directly to execute confirmed entries. Measured: 55-59% of every "confirmed" signal fell inside the session filter's blocked hours, bypassing the exact protection that got MTF from breakeven to profitable. **Fixed** (user asked for it once the bug was reported): `SessionFilteredSignalStrategy.get_confirmed_signal(tick_time)` now vetoes during blocked hours, using the confirming tick's candle-frame time; `SignalOrchestrator._on_tick` passes it, with a `TypeError` fallback for generators that don't accept it.
- Re-ran the full sweep against the real fix (not a manual correction) to check whether it changed the answer.
- **Window 1**: every n-tick config (2/3/5) beat baseline and cleared breakeven, peaking at N=3 (49.2% vs baseline 44.8%). **Window 2 (independent, non-overlapping)**: `--ntick 3` underperformed baseline and fell below breakeven (36.7% vs baseline 39.1%) -- flatly contradicting window 1, same as the first (manually-corrected) pass had shown.
- **Conclusion: no recommendation to enable.** The window-1 result doesn't survive an out-of-sample check, whether measured via the manual correction or the real fix -- looks like that window's specific price action, not a reproducible edge. `N_TICK_CONFIRMATION` stays at `1`. The bug fix ships regardless, on its own merits.
