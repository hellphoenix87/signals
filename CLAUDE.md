...
The app is wired together with plain factory functions (no DI framework). `app/factory.py` is the composition root: it builds `MarketData`, `Broker` (in `Mode.LIVE`), `RiskManager`, `TradeExecutor`, and per-symbol candle collector (`create_candle_collector` -> `LiveCandleCollector`/`MultiTimeframeCandleCollector`)/`TickCollector`/`SignalOrchestrator` instances (one orchestrator per entry in `Config.SYMBOLS`). `app/routes/endpoints.py` reaches those singletons through the `get_broker`/`get_market_data`/`get_trade_executor`/`get_orchestrators` getters via FastAPI `Depends()`, so routes can be exercised with overrides in tests -- but note `app/factory.py` still constructs everything eagerly at import time, so importing the routes module still builds a real `Broker`.

**Per-symbol orchestration (`app/services/trade_services.py::SignalOrchestrator`)** runs a hybrid loop:
- A background thread polls for newly *closed* entry-timeframe (`TF_ENTRY`, e.g. M1) candles per symbol. On each new closed candle: generate signals → update HTF bias on `exit_trade` → run candle-close profit exits → execute entries (via `trade_executor.process_signal`).
...
**Trade execution** (`app/trade_execution/`): all MT5 order calls should go through `broker.py` (`Broker`, created via `create_broker(mode)`) — `mode.py` defines `TradingMode.LIVE`/`BACKTEST` (backtest mode simulates positions in `broker.open_positions_sim` instead of sending orders, and requires an explicit historical `price`). `trade_execution.py` (`TradeExecutor`) orchestrates placing trades using `RiskManager` for sizing, gates entries on `Config.MAX_SPREAD_POINTS` via `_spread_ok`, and closes positions via `execute_exit`.
