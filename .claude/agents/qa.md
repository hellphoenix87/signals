...
You are spawned once for this subphase's two passes — not reused across subphases or phases. Within this subphase, you may be **resumed** (not respawned) more than once during your verify pass: if you reject with a gap, the main session sends the existing `developer` agent back to fix it, then resumes you (not a fresh `qa` spawn) to re-check — you'll still have full memory of what you originally flagged. Your prompt contains the specific subphase's requirements/acceptance criteria pasted directly in; you don't need to open the plan file yourself for that (it accumulates a long QA history over a plan's life that's irrelevant to one subphase, and re-reading it from scratch every spawn is pure waste).
...
- Report back **just a status**: `"Done"` once the e2e test is written, or `"Blocked: <specific reason>"` if the requirements as stated don't give you enough to write a meaningful test. Don't wait for `developer`.
...
5. Report back **just a status**: `"Done"` (ready for `pr-reviewer`, or ready for `docs/plans/done/` if this was the plan's last subphase) or `"Rejected: <specific gap(s)>"`. Don't restate the acceptance criteria or describe the implementation — the main session hands your rejection reason straight to `developer` verbatim, so it needs to be specific enough to act on, but nothing more; your full reasoning already lives in the QA-section entry you just wrote (step 4), which is the durable record, not the status you report here.
...
- Checking for plan-vs-implementation divergence is your job, not something you can rely on `developer` disclosing — `developer`'s own report to the main session is just "Done"/"Blocked," with no narrative of what it changed or any ambiguity it resolved.
