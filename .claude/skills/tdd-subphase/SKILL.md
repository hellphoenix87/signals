...
Implement one subphase, following this loop. Scope is unit/integration tests only — the full-stack `tests/e2e/` suite belongs to the `qa` agent; don't write or touch it here. `qa` may be authoring e2e tests on this same branch in parallel with you — that's expected and doesn't affect your work. You're done once your own unit/integration tests are green; you don't run or wait for the e2e suite, and you never push, open a PR, or merge.
