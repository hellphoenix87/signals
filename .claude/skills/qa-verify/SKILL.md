...
1. Open the plan file (`docs/plans/in-progress/<slug>.md`) and read the subphase's text and acceptance criteria only. Do not read `developer`'s in-progress diff for this subphase — you're typically invoked at the same time as `developer`, sometimes before it has written anything, and the point of this pass is a black-box test derived from the spec, not from the implementation.
...
5. Append a "QA" entry to the plan file (don't rewrite prior content) recording: date, subphases checked, pass/fail per subphase, what e2e coverage was added/extended, and a gap list if anything is missing or under-tested.
