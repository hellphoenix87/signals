...
class SignalOrchestrator:
    ...
    def _on_tick(self, tick: Any) -> None:
        ...
        symbol = "EURUSD"  # <-- hardcoded symbol, or extract from tick if available
        ...
        if hasattr(self.signal_generator, "on_new_tick"):
            try:
                self.signal_generator.on_new_tick(price, spread_points, symbol)
            except Exception as exc:
                self._log_exception(
                    f"[Orchestrator] signal_generator.on_new_tick error: {exc!r}"
                )

        # Call generate_signal on every tick, passing minimal candle with symbol
        if hasattr(self.signal_generator, "generate_signal"):
            try:
                candles = [{"symbol": symbol}]
                sig = self.signal_generator.generate_signal(candles)
                ...
                self._log_exception(
                    f"[Orchestrator] signal_generator.generate_signal error: {exc!r}"
                )
